...
impl PeerSamplingService {
        ...
        tokio::spawn((|| async move {
            while let Some((sender_address, kind, peers)) = receiver.recv().await {
                ...
                if kind == MessageKind::Request && *config.pull() {
                    let buffer = Self::build_local_view_buffer(&config, view.clone());
                    ...
                    .await
                    {
                        Ok(written) => hb_log::info(None, &format!("[ApiInternalGossip] Peer sampling with local view response sent successfully to {sender_address} ({buffer_count} peers, {written} bytes)")),
            ...
            loop {
                ...
                if let Some(peer) = view.select_peer() {
                    if *config.push() {
                        ...
                        .await
                        {
                            Ok(written) => hb_log::info(None, &format!("[ApiInternalGossip] Peer sampling with local view request sent successfully to {} ({} peers, {} bytes)", peer.address(), buffer_count, written)),
    ...
    fn build_local_view_buffer(config: &PeerSamplingConfig, view: View) -> Vec<Peer> {
        let mut view = view.add_with_local();
        view.permute();
