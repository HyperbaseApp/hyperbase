...
pub async fn init(pool: &Pool<MySql>) {
    ...
    pool.execute("CREATE TABLE IF NOT EXISTS `admins` (`id` varchar(36), `created_at` timestamp, `updated_at` timestamp, `email` text, `password_hash` text, PRIMARY KEY (`id`))").await.unwrap();
