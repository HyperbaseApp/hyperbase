...
pub async fn init(pool: &Pool<MySql>) {
    ...
    pool.execute("CREATE TABLE IF NOT EXISTS `admin_password_resets` (`id` varchar(36), `created_at` timestamp, `updated_at` timestamp, `admin_id` varchar(36), `code` text, PRIMARY KEY (`id`))").await.unwrap();
