...
pub async fn init(pool: &Pool<MySql>) {
    ...
    pool.execute("CREATE TABLE IF NOT EXISTS `collections` (`id` binary(16), `created_at` timestamp, `updated_at` timestamp, `project_id` binary(16), `name` text, `schema_fields` json, `opt_auth_column_id` boolean, `opt_ttl` bigint, PRIMARY KEY (`id`))").await.unwrap();
