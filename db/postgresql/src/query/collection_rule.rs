...
const SELECT_MANY_BY_TOKEN_ID: &str = "SELECT \"id\", \"created_at\", \"updated_at\", \"project_id\", \"token_id\", \"collection_id\", \"find_one\", \"find_many\", \"insert_one\", \"update_one\", \"delete_one\" FROM \"collection_rules\" WHERE \"token_id\" = $1";
...
pub async fn init(pool: &Pool<Postgres>) {
    ...
    pool.execute("CREATE TABLE IF NOT EXISTS \"collection_rules\" (\"id\" uuid, \"created_at\" timestamptz, \"updated_at\" timestamptz, \"project_id\" uuid, \"token_id\" uuid, \"collection_id\" uuid, \"find_one\" boolean, \"find_many\" boolean, \"insert_one\" boolean, \"update_one\" boolean, \"delete_one\" boolean, PRIMARY KEY (\"id\"))").await.unwrap();
